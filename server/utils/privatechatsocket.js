...
function privateChat(io) {
  io.on("connection", (socket) => {
    ...
    console.log(`User '${socket.handshake.auth.username}' connect private`);
    ...
    socket.on("one_send_message", (data) => {
      ...
      const roomname = clientArry[userid]["privateroom"]
        ? clientArry[userid]["privateroom"]
        : null;
    ...
    socket.on("disconnect", () => {
      ...
      removeClient(auth);
      console.log(`User '${auth.username}' disconnect private`);
    });
  });
}
